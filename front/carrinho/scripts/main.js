...
function gerarQRCodePIX(chavePIX) {
  rl.question('Digite o valor do pagamento PIX: ', function (valor) {
    valor = parseFloat(valor)
    const qrCodeData = `00020101021126560014BR.GOV.BCB.PIX0114${chavePIX}52040000530398654060.005802BR5913Nome do Beneficiario6009SAO PAULO62290510Unidade Federativa6304${valor.toFixed(
      2,
    )}64530201.1907`
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function abrirPopUpPIX() {
    const popup = window.open('', 'Pagamento PIX', 'width=400,height=400')
    ...
    popup.document
      ...
      .addEventListener('click', function () {
        const nome = popup.document.getElementById('nome').value
        const endereco = popup.document.getElementById('endereco').value
        const numero = popup.document.getElementById('numero').value
        const cep = popup.document.getElementById('cep').value

        gerarQRCodePIX(
          'a62578eb-4d7e-4b0c-888f-90eb731dbada',
          valorTotal,
          nome,
          endereco,
          numero,
  ...
  function gerarQRCodePIX(chavePIX, valor, nome, endereco, numero, cep, popup) {
    const qrCodeData = `00020101021126560014BR.GOV.BCB.PIX0114${chavePIX}52040000530398654060.005802BR5913${nome}6009${endereco}6229${numero}6304${cep}6304${valor.toFixed(
      2,
    )}64530201.1907`
